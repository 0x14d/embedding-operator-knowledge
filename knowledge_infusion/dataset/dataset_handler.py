...
class DatasetHandler:
    ...
    def _filter_data_by_info_tag(self, info_tag: str) -> None:
        ...
        if info_tag == 'process_parameters':
            process_parameters = [
                process_param for process_param in sorted(list(self._info[info_tag]))
            ...
            self._experiment_parameters = self._normalize_dataframe_and_add_id(
                process_params_df, info_tag
            )
        elif info_tag == 'ratings':
            ratings = sorted(list(self._info[info_tag]))
    ...
    def _check_experiments_completeness(self) -> None:
        """
        check if contiguous experiments hold all necessary data
        in the dataframe and provide correct tensor shape
        :return: None
        """
        ...
        for experiment_tuple in self._contiguous_experiments:
            ...
            elif self._dataset_type in ['ratings2param', 'ratings&param2param']:
            ...
            label_experiment = self._experiment_parameters.loc[
                self._experiment_parameters['ID'] == experiment_tuple[1]
            ]
