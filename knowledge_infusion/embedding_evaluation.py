...
class EmbeddingEvaluation:
    ...
    def __init__(self, distance_measure: str, use_head: bool, path='knowledge_infusion/config/default_config.json',
                 influential_only=False, ratings: Optional[List[str]] = None, train_config_obj = None, embedding_config= None):
        """
    ...
    def generate_rating_param_sub_kg(self):
        ...
        for rating in self._ratings:
            ...
            if subkg_emb is not None:
                params = subkg_emb.get_parameters_in_sub_kg()
                for p in all_params:
                    if p in params:
                        param_dict[p] = True
                    else:
        ...
        for data in data_loader:
            ...
            for _, feature in enumerate(inputs):
                ...
                if subgraph_emb is not None:
                    params = subgraph_emb.get_parameters_in_sub_kg()
                    param_dict = {}
                    for p in all_params:
                        if p in params:
                            param_dict[p] = True
