...
def create_df(data, side):
    ...
    columns = pd.MultiIndex.from_product(
        [kgs, ['AMRI', 'Hits@1', 'Hits@5', 'Hits@10']],
        names=['Representations', 'Metrics']
    )
    ...
    for measurement in data:
        ...
        try:
            df.at[method, (kg_type, 'AMRI')] = round(
                measurement['results']['adjustedarithmeticmeanrankindex', side], 2)
            df.at[method, (kg_type, 'Hits@1')
                  ] = round(measurement['results']['hits_at_1', side], 2)
            df.at[method, (kg_type, 'Hits@5')
                  ] = round(measurement['results']['hits_at_5', side], 2)
            df.at[method, (kg_type, 'Hits@10')
                  ] = round(measurement['results']['hits_at_10', side], 2)
        except TypeError:
            df.at[method, (kg_type, 'AMRI')
                  ] = measurement['results']['adjustedarithmeticmeanrankindex', side]
            df.at[method, (kg_type, 'Hits@1')
                  ] = measurement['results']['hits_at_1', side]
            df.at[method, (kg_type, 'Hits@5')
                  ] = measurement['results']['hits_at_5', side]
            df.at[method, (kg_type, 'Hits@10')
                  ] = measurement['results']['hits_at_10', side]
...
def combine_multiple_iterations(iteration_results, with_std=False):
    ...
    for result in og:
        ...
        for key in result['results'].keys():
            ...
            mean_s = mean(similar)
            if with_std:
                result['results'][key] = str(
                    round(mean_s, 2)) + '±' + str(round(stdev(similar), 2))
            else:
...
if __name__ == '__main__':
    result_path = "knowledge_infusion/compare_methods/results/LinkPrediction/*/*/*/"
    ...
    for base_path in exps:
        ...
        save_all_tables(frame, base_path +
                        "combined_eval_results_no_std/", 'head')

        frame = create_df(multi, 'tail')
        save_all_tables(frame, base_path +
                        "combined_eval_results_no_std/", 'tail')

        multi = combine_multiple_iterations(collected_data, with_std=True)
        frame = create_df(multi, 'head')
        save_all_tables(frame, base_path +
                        "combined_eval_results_with_std/", 'head')

        frame = create_df(multi, 'tail')
        save_all_tables(frame, base_path +
                        "combined_eval_results_with_std/", 'tail')
